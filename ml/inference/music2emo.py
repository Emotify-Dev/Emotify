...
import gradio as gr
from utils.pytorch_utils import get_inference_device, checkpoint_map_location
...
def split_audio(waveform, sample_rate):
    ...
    # MERT CNN feature extractor (wav2vec2, kernel sizes [10,3,3,3,3,2,2], no padding)
    # needs ≥720 samples to produce ≥2 transformer tokens (seq_len=1 causes a squeeze
    # dimension bug). 4800 samples (200 ms) is used as a practical floor — anything
    # shorter is a meaningless trailing fragment and not worth including in the average.
    min_segment_samples = 4800
    segments = []
...
_SAVED_MODELS_DIR = _INFERENCE_DIR.parent / "saved_models"
...
class Music2emo:
    def __init__(
        ...
        if model_weights is None:
            model_weights = str(_SAVED_MODELS_DIR / "J_all.ckpt")
        self.device = get_inference_device()

        self.feature_extractor = FeatureExtractorMERT(model_name='m-a-p/MERT-v1-95M', device=self.device,
                                                      sr=resample_rate)
        ...
        checkpoint = torch.load(
            self.model_weights,
            map_location=checkpoint_map_location(self.device),
            weights_only=False,
        )
    ...
    def predict(self, audio, threshold=0.5):
        ...
        # Use a private temp directory — the original code used relative paths
        # (./temp_out, ./output) which could delete user files in the CWD.
        _tmp = Path(tempfile.mkdtemp(prefix="music2emo_"))
        try:
            feature_dir = _tmp / "features"
            ...
            # torchaudio 2.9+ requires torchcodec; use librosa (already a dependency) instead.
            import librosa as _librosa
            _wav_np, sample_rate = _librosa.load(audio, sr=None, mono=True)
            waveform = torch.from_numpy(_wav_np)
            ...
            for filename in sorted(os.listdir(mert_dir)):
                ...
                if os.path.isfile(file_path) and filename.endswith('.npy'):
                    ...
                    concatenated_features = np.squeeze(concatenated_features)  # 768 * 2 = 1536
            ...
            config = HParams.load(str(_INFERENCE_DIR / "config" / "run_config.yaml"))
            config.feature['large_voca'] = True
            config.model['num_chords'] = 170
            model_file = str(_SAVED_MODELS_DIR / "btc_model_large_voca.pt")
            idx_to_chord = idx2voca_chord()
            model = BTC_model(config=config.model).to(self.device)

            if os.path.isfile(model_file):
                checkpoint = torch.load(model_file, map_location=checkpoint_map_location(self.device), weights_only=False)
            ...
            with open(_INFERENCE_DIR / "data" / "chord.json", 'r') as f:
                chord_to_idx = json.load(f)
            with open(_INFERENCE_DIR / "data" / "chord_inv.json", 'r') as f:
                idx_to_chord = json.load(f)
                idx_to_chord = {int(k): v for k, v in idx_to_chord.items()}
            with open(_INFERENCE_DIR / "data" / "chord_root.json") as json_file:
                chordRootDic = json.load(json_file)
            with open(_INFERENCE_DIR / "data" / "chord_attr.json") as json_file:
            ...
            mode = "major" if key_signature == "None" else key_signature.split()[-1]
            ...
            encoded, encoded_root, encoded_attr, durations = [], [], [], []
            ...
            tag_list = np.load(str(_INFERENCE_DIR / "data" / "tag_list.npy"))
            ...
            return {"valence": valence, "arousal": arousal, "predicted_moods": predicted_moods}
